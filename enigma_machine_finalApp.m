classdef enigma_machine_finalApp < handle
    %
    % Usage:
    %     app.enigma_machine_finalApp
    ...
    properties (Constant)
        AppPathBase = {'/enigma_machine_final'};
        AppClass = 'enigma_machine_finalApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = enigma_machine_finalApp()
        ...
        function startApp(obj)
            ...
            mlock;
            enigma_machine_finalApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'enigma_app')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong enigma_app
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'enigma_app').getString, ...
            ...
            if nargout(@enigma_app) == 0
                eval('enigma_app');
            else
                obj.AppHandle = eval('enigma_app');
            end
